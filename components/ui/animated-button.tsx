...
const buttonVariants = cva("group inline-flex w-full items-center justify-center rounded-full font-medium transition-all sm:w-auto", {
  variants: {
    variant: {
      default:
...
export function AnimatedButton({ href, children, variant, size, className, openInNewTab = false }: AnimatedButtonProps) {
...
export function GradientBorder({ children, className, variant = "default" }: GradientBorderProps) {
  ...
  return (
    <span className={cn("relative inline-block overflow-hidden rounded-full p-0.5", className)}>
      ...
      />
      <div className="inline-flex h-full w-full items-center justify-center rounded-full bg-white backdrop-blur-3xl dark:bg-gray-950">{children}</div>
    </span>
  );
}
